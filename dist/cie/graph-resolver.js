...
const ENGINEERING_SECURITY_PHASE = [
    {
...
// ─── Resolver ────────────────────────────────────────────────────
function resolveExecutionGraph(department, task, agentId = '', entityId) {
    let stages = [];
    const warnings = [];
...
function isEngineeringTask(task) {
    const engKeywords = [
...
function isSecuritySensitive(task, entityId) {
    if (entityId) {
        try {
            const radius = (0, graphify_1.getImpactRadius)(entityId, { hops: 1 });
            const hit = radius.find(r => SECURITY_SENSITIVE_PATH_HINTS.some(h => (r.node.source_file || r.node.id || '').toLowerCase().includes(h)));
            if (hit) {
