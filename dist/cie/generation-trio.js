...
exports.shouldRunFullTrio = shouldRunFullTrio;
exports.setHttpPost = setHttpPost;
exports.runGenerationTrio = runGenerationTrio;
...
const FULL_TRIO_ARCHETYPES = ['PRECISION_CRITICAL', 'ADVERSARIAL_TESTING'];
function shouldRunFullTrio(archetype) {
    return FULL_TRIO_ARCHETYPES.includes(archetype);
}
const realHttpPost = (url, opts) => fetch(url, { method: 'POST', headers: opts.headers, body: opts.body });
let httpPost = realHttpPost;
/** Swap the HTTP layer for tests. Call with no args to restore the real fetch. */
function setHttpPost(fn) {
    httpPost = fn ?? realHttpPost;
}
// ─── Provider calls ──────────────────────────────────────────────────────
async function callAnthropicCompatible(input, role, opts) {
    if (!opts.apiKey) {
...
async function callOpenAiCompatible(input, role, opts) {
    if (!opts.apiKey) {
        return { role, available: false, provider: opts.provider, model: opts.model, reason: `no API key configured for ${opts.provider}` };
    }
    try {
 ...
 */
async function runGenerationTrio(archetype, input) {
...
//# sourceMappingURL=generation-trio.js.map