...
exports.gatherCreativeContext = gatherCreativeContext;
 ...
 */
function gatherCreativeContext(query, opts = {}) {
    const looseGraphEdges = opts.entityId ? (0, graphify_1.getLooseNeighbors)(opts.entityId) : [];
    const scoped = (0, mempalace_1.searchMemPalace)(query, { wing: opts.wing, room: opts.room, results: opts.results ?? 5 });
    const distant = (0, mempalace_1.searchMemPalace)(query, { results: opts.results ?? 5 });
    return {
        looseGraphEdges,
        scoped,
        distant,
        kaiPerformance: null,
        kaiUnavailableReason: 'kai (Teams/Brand Studio/kai) is a prompt-only agent definition — no scored historical ' +
            'performance model or dataset exists in code (verified 2026-08-09)',
    };
}
//# sourceMappingURL=creative-retrieval.js.map