...
exports.bridgeCrossScopeQuery = bridgeCrossScopeQuery;
const ventures_1 = require("./sources/ventures");
const mempalace_1 = require("./sources/mempalace");
const BRIDGE_ELIGIBLE_KINDS = ['core', 'venture'];
/**
 ...
 */
async function bridgeCrossScopeQuery(sourceSlug, query, opts = {}) {
    const ventures = await (0, ventures_1.listVentures)({ skipCache: opts.skipCache });
    ...
    const siblings = ventures.filter((v) => v.slug !== sourceSlug && BRIDGE_ELIGIBLE_KINDS.includes(v.kind) && v.status === 'active');
    ...
    for (const sibling of siblings) {
        const result = (0, mempalace_1.searchMemPalace)(query, { wing: sibling.slug, results: opts.results ?? 5 });
...
//# sourceMappingURL=cross-scope-bridge.js.map