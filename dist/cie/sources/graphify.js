...
exports.getLooseNeighbors = getLooseNeighbors;
...
function invalidateGraphifyCache() {
...
function getGraphData() {
    ...
    if (!(0, fs_1.existsSync)(path))
        return null;
    const mtime = (0, fs_1.statSync)(path).mtimeMs;
    if (cachedGraphData && cachedGraphDataMtime === mtime)
        return cachedGraphData;
    let parsed;
    try {
        parsed = JSON.parse((0, fs_1.readFileSync)(path, 'utf-8'));
    }
    catch {
        return null;
    }
 ...
 */
function queryGraph(entity, scope) {
 ...
 * omit for both).
 */
function getNeighbors(entityId, edgeTypes, opts = {}) {
    const data = getGraphData();
    if (!data)
        return [];
    ...
    const walk = (id) => {
        ...
        if (direction === 'out' || direction === 'both')
            edges.push(...(data.outEdges.get(id) ?? []));
        if (direction === 'in' || direction === 'both')
            edges.push(...(data.inEdges.get(id) ?? []));
        for (const edge of edges) {
            if (typeSet && !typeSet.has(edge.relation))
                continue;
            if (confSet && !confSet.has(edge.confidence ?? ''))
                continue;
 ...
 */
function getLooseNeighbors(entityId, opts = {}) {
...
function getImpactRadius(entityId, opts = {}) {
