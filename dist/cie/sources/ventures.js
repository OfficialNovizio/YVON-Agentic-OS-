...
exports.invalidateVenturesCache = invalidateVenturesCache;
 ...
 * not throw.
 */
async function listVentures(opts = {}) {
    if (!opts.skipCache && cachedVentures && Date.now() - cachedAt < CACHE_TTL_MS)
        return cachedVentures;
    const env = supabaseEnv();
    if (!env)
        return [];
    try {
        const res = await fetch(`${env.url}/rest/v1/ventures?select=id,name,slug,kind,repo_url,local_repo_path,status`, { headers: { apikey: env.key, Authorization: `Bearer ${env.key}` } });
        if (!res.ok)
            return [];
        ...
        const ventures = rows.map((r) => ({
            ...
            localRepoPath: r.local_repo_path ?? null,
...
function invalidateVenturesCache() {
    cachedVentures = null;
    cachedAt = 0;
}
