...
exports.executeGovernanceReview = executeGovernanceReview;
exports.requiresGovernanceReview = requiresGovernanceReview;
 ...
 */
async function executeGovernanceReview(decisionId, decisionDescription, venture = 'default') {
    const t0 = Date.now();
    const log = [];
    const gates = [];
    ...
    try {
        const precedentCtx = await (0, cie_1.buildCieContext)({
    ...
    for (const gate of GATE_SEQUENCE) {
        ...
        try {
            const cieCtx = await (0, cie_1.buildCieContext)({
 ...
 */
function requiresGovernanceReview(spendAmount, gateThreshold, touchesConstitution = false, touchesLockedCommitment = false) {
    return spendAmount > gateThreshold || touchesConstitution || touchesLockedCommitment;
}
//# sourceMappingURL=governance-gate.js.map