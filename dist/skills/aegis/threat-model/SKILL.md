...
tier: 2
description: |
  A threat model answers **'what could go wrong with this system, who would do it, and what should we do about it?'** independently of whether any specific bug has been found yet. (yvon)
triggers:
  - threat model
allowed-tools:
  - Read
...
identity: none
provenance:
  source_file: Teams/Engineering/aegis/marketplace/threat-model/SKILL.md
  source_hash: cb4c1c158315956ae033f97670aa0a644eb53551df698ab8e48f37845ef38d98
  generated: 2026-07-20T03:20:22.450Z
  generator: cli/skillgen.js
portable: true
---
<!-- AUTO-GENERATED from Teams/Engineering/aegis/marketplace/threat-model/SKILL.md — DO NOT EDIT.
...
echo "OPERATING AS: aegis — Engineering · skill: threat-model"
echo "REPO_ROOT: $_ROOT"
_BRANCH=$(git -C "$_ROOT" branch --show-current 2>/dev/null || echo unknown)
echo "BRANCH: $_BRANCH"
mkdir -p "$_ROOT/store/telemetry" 2>/dev/null || true
echo "{\"ts\":\"$(date -u +%FT%TZ)\",\"agent\":\"aegis\",\"skill\":\"threat-model\",\"branch\":\"$_BRANCH\"}" >> "$_ROOT/store/telemetry/skill-invocations.jsonl" 2>/dev/null || true
```
...
Use when the request matches: "threat model".
...
_ROOT=$(pwd); while [ "$_ROOT" != "/" ] && [ ! -d "$_ROOT/Teams" ]; do _ROOT=$(dirname "$_ROOT"); done
echo "{\"ts\":\"$(date -u +%FT%TZ)\",\"agent\":\"aegis\",\"skill\":\"threat-model\",\"event\":\"complete\",\"outcome\":\"$_OUTCOME\"}" >> "$_ROOT/store/telemetry/skill-invocations.jsonl" 2>/dev/null || true
```
