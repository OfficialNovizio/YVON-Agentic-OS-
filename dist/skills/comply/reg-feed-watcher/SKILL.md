...
tier: 3
description: |
  Check regulatory feeds now and report what's new since the last check, filtered by your materiality threshold. Use when the user says "check the feeds", "what's new", "regulatory update", when running from the scheduled agent, or when manually pasting a regulatory development for classification and diff. (yvon)
triggers:
  - reg feed watcher
allowed-tools:
  - Write
...
provenance:
  source_file: Teams/Legal & Compliance/comply/marketplace/reg-feed-watcher/SKILL.md
  source_hash: 5a6ec2fe5efe25e105cbb4db99a4928bcfd687b48b40f3ac58bdd00b85c97fc4
  generated: 2026-07-30T16:55:24.908Z
  generator: cli/skillgen.js
portable: true
---
<!-- AUTO-GENERATED from Teams/Legal & Compliance/comply/marketplace/reg-feed-watcher/SKILL.md — DO NOT EDIT.
...
echo "OPERATING AS: comply — Legal & Compliance · skill: reg-feed-watcher"
echo "REPO_ROOT: $_ROOT"
_BRANCH=$(git -C "$_ROOT" branch --show-current 2>/dev/null || echo unknown)
echo "BRANCH: $_BRANCH"
mkdir -p "$_ROOT/store/telemetry" 2>/dev/null || true
echo "{\"ts\":\"$(date -u +%FT%TZ)\",\"agent\":\"comply\",\"skill\":\"reg-feed-watcher\",\"branch\":\"$_BRANCH\"}" >> "$_ROOT/store/telemetry/skill-invocations.jsonl" 2>/dev/null || true
...
Use when the request matches: "reg feed watcher".
...
_ROOT=$(pwd); while [ "$_ROOT" != "/" ] && [ ! -d "$_ROOT/Teams" ]; do _ROOT=$(dirname "$_ROOT"); done
echo "{\"ts\":\"$(date -u +%FT%TZ)\",\"agent\":\"comply\",\"skill\":\"reg-feed-watcher\",\"event\":\"complete\",\"outcome\":\"$_OUTCOME\"}" >> "$_ROOT/store/telemetry/skill-invocations.jsonl" 2>/dev/null || true
```
