...
tier: 2
description: |
  Design or evaluate the visual artifacts that express a brand: logo system, color, typography, imagery, iconography, and motion. (yvon)
triggers:
  - brand identity
allowed-tools:
  - Write
...
identity: none
provenance:
  source_file: Teams/Brand Studio/atlas/marketplace/brand-identity/SKILL.md
  source_hash: f15e91a99cfef9c6a13cf8e56f952b318eebbada0ab10a2928d855f502a094df
  generated: 2026-07-20T03:20:23.526Z
  generator: cli/skillgen.js
portable: true
---
<!-- AUTO-GENERATED from Teams/Brand Studio/atlas/marketplace/brand-identity/SKILL.md — DO NOT EDIT.
...
echo "OPERATING AS: atlas — Brand Studio · skill: brand-identity"
echo "REPO_ROOT: $_ROOT"
_BRANCH=$(git -C "$_ROOT" branch --show-current 2>/dev/null || echo unknown)
echo "BRANCH: $_BRANCH"
mkdir -p "$_ROOT/store/telemetry" 2>/dev/null || true
echo "{\"ts\":\"$(date -u +%FT%TZ)\",\"agent\":\"atlas\",\"skill\":\"brand-identity\",\"branch\":\"$_BRANCH\"}" >> "$_ROOT/store/telemetry/skill-invocations.jsonl" 2>/dev/null || true
```
...
_ROOT=$(pwd); while [ "$_ROOT" != "/" ] && [ ! -d "$_ROOT/Teams" ]; do _ROOT=$(dirname "$_ROOT"); done
echo "{\"ts\":\"$(date -u +%FT%TZ)\",\"agent\":\"atlas\",\"skill\":\"brand-identity\",\"event\":\"complete\",\"outcome\":\"$_OUTCOME\"}" >> "$_ROOT/store/telemetry/skill-invocations.jsonl" 2>/dev/null || true
```
