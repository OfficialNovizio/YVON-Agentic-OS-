...
tier: 3
description: |
  Incidents are inevitable — dev's identity assumes everything fails all the time. (yvon)
triggers:
  - incident response
  - incident
  - production is down
...
identity: none
provenance:
  source_file: Teams/Engineering/ops/custom/incident-response/SKILL.md
  source_hash: 7505c88f20deba20c6aecb72bbd3899017927ba3598f3f68b57e5e9a2e45e553
  generated: 2026-07-20T03:20:22.791Z
  generator: cli/skillgen.js
portable: true
---
<!-- AUTO-GENERATED from Teams/Engineering/ops/custom/incident-response/SKILL.md — DO NOT EDIT.
...
echo "OPERATING AS: ops — Engineering · skill: incident-response"
echo "REPO_ROOT: $_ROOT"
_BRANCH=$(git -C "$_ROOT" branch --show-current 2>/dev/null || echo unknown)
echo "BRANCH: $_BRANCH"
mkdir -p "$_ROOT/store/telemetry" 2>/dev/null || true
echo "{\"ts\":\"$(date -u +%FT%TZ)\",\"agent\":\"ops\",\"skill\":\"incident-response\",\"branch\":\"$_BRANCH\"}" >> "$_ROOT/store/telemetry/skill-invocations.jsonl" 2>/dev/null || true
...
_ROOT=$(pwd); while [ "$_ROOT" != "/" ] && [ ! -d "$_ROOT/Teams" ]; do _ROOT=$(dirname "$_ROOT"); done
echo "{\"ts\":\"$(date -u +%FT%TZ)\",\"agent\":\"ops\",\"skill\":\"incident-response\",\"event\":\"complete\",\"outcome\":\"$_OUTCOME\"}" >> "$_ROOT/store/telemetry/skill-invocations.jsonl" 2>/dev/null || true
```
