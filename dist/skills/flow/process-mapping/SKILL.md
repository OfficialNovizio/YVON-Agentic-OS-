...
tier: 3
description: |
  Map any recurring process — steps, owners, inputs, outputs, cycle time, wait time. Uses Deming's SIPOC + value-stream discipline. Never invents steps. (yvon)
triggers:
  - process mapping
allowed-tools:
  - Read
...
provenance:
  source_file: Teams/Ops & Delivery/flow/custom/process-mapping/SKILL.md
  source_hash: f6a1a5b5ec90db3445eab9374698ad9e4f250568e2f423d5068099283c2ff615
  generated: 2026-08-08T17:03:37.309Z
  generator: cli/skillgen.js
portable: true
---
<!-- AUTO-GENERATED from Teams/Ops & Delivery/flow/custom/process-mapping/SKILL.md — DO NOT EDIT.
...
echo "OPERATING AS: flow — Ops & Delivery · skill: process-mapping"
echo "REPO_ROOT: $_ROOT"
_BRANCH=$(git -C "$_ROOT" branch --show-current 2>/dev/null || echo unknown)
echo "BRANCH: $_BRANCH"
mkdir -p "$_ROOT/store/telemetry" 2>/dev/null || true
echo "{\"ts\":\"$(date -u +%FT%TZ)\",\"agent\":\"flow\",\"skill\":\"process-mapping\",\"branch\":\"$_BRANCH\"}" >> "$_ROOT/store/telemetry/skill-invocations.jsonl" 2>/dev/null || true
...
Use when the request matches: "process mapping".
...
_ROOT=$(pwd); while [ "$_ROOT" != "/" ] && [ ! -d "$_ROOT/Teams" ]; do _ROOT=$(dirname "$_ROOT"); done
echo "{\"ts\":\"$(date -u +%FT%TZ)\",\"agent\":\"flow\",\"skill\":\"process-mapping\",\"event\":\"complete\",\"outcome\":\"$_OUTCOME\"}" >> "$_ROOT/store/telemetry/skill-invocations.jsonl" 2>/dev/null || true
```
