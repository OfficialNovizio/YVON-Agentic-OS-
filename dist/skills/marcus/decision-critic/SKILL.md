...
tier: 3
description: |
  Give the operator (marcus, or whoever marcus is advising) a sharper decision — not a longer risk list. (yvon)
triggers:
  - decision critic
  - big decision
  - stress test this
...
provenance:
  ...
  source_hash: 303b7244ef8448cbed66898d4f2b6528517e569cc3385698178613f5aafd9b7f
  generated: 2026-07-20T03:20:24.204Z
  generator: cli/skillgen.js
portable: true
---
<!-- AUTO-GENERATED from Teams/Executive Office/marcus/custom/decision-critic/SKILL.md — DO NOT EDIT.
...
echo "OPERATING AS: marcus — Executive Office · skill: decision-critic"
echo "REPO_ROOT: $_ROOT"
_BRANCH=$(git -C "$_ROOT" branch --show-current 2>/dev/null || echo unknown)
echo "BRANCH: $_BRANCH"
mkdir -p "$_ROOT/store/telemetry" 2>/dev/null || true
echo "{\"ts\":\"$(date -u +%FT%TZ)\",\"agent\":\"marcus\",\"skill\":\"decision-critic\",\"branch\":\"$_BRANCH\"}" >> "$_ROOT/store/telemetry/skill-invocations.jsonl" 2>/dev/null || true
...
_ROOT=$(pwd); while [ "$_ROOT" != "/" ] && [ ! -d "$_ROOT/Teams" ]; do _ROOT=$(dirname "$_ROOT"); done
echo "{\"ts\":\"$(date -u +%FT%TZ)\",\"agent\":\"marcus\",\"skill\":\"decision-critic\",\"event\":\"complete\",\"outcome\":\"$_OUTCOME\"}" >> "$_ROOT/store/telemetry/skill-invocations.jsonl" 2>/dev/null || true
```
