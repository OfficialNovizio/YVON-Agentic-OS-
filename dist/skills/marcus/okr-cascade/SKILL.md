...
tier: 3
description: |
  Give marcus (or whoever marcus is advising) a repeatable way to go from "here's our priority this quarter" to a small set of inspiring, measurable objectives at the top level, and defensible, aligned… (yvon)
triggers:
  - okr cascade
  - set okrs
  - quarterly planning
...
provenance:
  source_file: Teams/Executive Office/marcus/custom/okr-cascade/SKILL.md
  source_hash: 649e7ed9ccded4bf425329344ea09991d8fc9f3f554b4f96a333162e3fdd8096
  generated: 2026-07-20T03:20:24.209Z
  generator: cli/skillgen.js
portable: true
---
<!-- AUTO-GENERATED from Teams/Executive Office/marcus/custom/okr-cascade/SKILL.md — DO NOT EDIT.
...
echo "OPERATING AS: marcus — Executive Office · skill: okr-cascade"
echo "REPO_ROOT: $_ROOT"
_BRANCH=$(git -C "$_ROOT" branch --show-current 2>/dev/null || echo unknown)
echo "BRANCH: $_BRANCH"
mkdir -p "$_ROOT/store/telemetry" 2>/dev/null || true
echo "{\"ts\":\"$(date -u +%FT%TZ)\",\"agent\":\"marcus\",\"skill\":\"okr-cascade\",\"branch\":\"$_BRANCH\"}" >> "$_ROOT/store/telemetry/skill-invocations.jsonl" 2>/dev/null || true
...
_ROOT=$(pwd); while [ "$_ROOT" != "/" ] && [ ! -d "$_ROOT/Teams" ]; do _ROOT=$(dirname "$_ROOT"); done
echo "{\"ts\":\"$(date -u +%FT%TZ)\",\"agent\":\"marcus\",\"skill\":\"okr-cascade\",\"event\":\"complete\",\"outcome\":\"$_OUTCOME\"}" >> "$_ROOT/store/telemetry/skill-invocations.jsonl" 2>/dev/null || true
```
