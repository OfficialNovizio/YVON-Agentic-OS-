---
# --- original source frontmatter (verbatim from anthropics/claude-for-legal) ---
...
# --- YVON provenance (added per playbook §9, §11) ---
type: marketplace
status: copied verbatim
source: https://github.com/anthropics/claude-for-legal/tree/main/commercial-legal/skills/vendor-agreement-review
source_raw: https://raw.githubusercontent.com/anthropics/claude-for-legal/main/commercial-legal/skills/vendor-agreement-review/SKILL.md
source_repo: anthropics/claude-for-legal
source_repo_license: See upstream repository
author: anthropics (Anthropic PBC)
fulfills_catalog_entry: contract-drafting / contract-review (Legal & Compliance · scribe · Contracts)
assigned_agent: scribe (Legal & Compliance / Contracts)
portable: false            # jurisdictional references (CA/NY/IL/EU/UK) and hardcoded plugin config path; wrapped by scribe/custom/contract-review-routing to rebind config onto operational/agent/scribe-config.md
date_added: 2026-07-29

# --- YVON compile metadata (playbook §14.3, §14.4) ---
tier: 2                    # advisory (marketplace default per §14.3)
triggers:
  ...
  - redline this vendor contract
  - is this vendor contract okay
  - check the vendor terms
---

<!--
YVON selection rationale (playbook §4.3, §11)

Selected 2026-07-29 as scribe's contract-review skill.

Why this over the alternatives:
- vs. anthropics/knowledge-work-plugins/contract-review (SMB): the SMB skill bakes market
  defaults into the body ("Net-30 is standard", "2-3 years is typical") — invented values
  presented as fact (playbook §0.5), and unreachable to per-venture config without editing
  the skill (playbook §4.8 forbids). This skill is config-driven — it bounces when the
  playbook is missing, or runs [PROVISIONAL] and tags outputs accordingly. Matches YVON's
  operational/agent/<agent>-config.md pattern (§7, §14.7).

- Refuses to guess dollar values, refuses to silently supplement thin research results,
  and requires every citation to carry provenance tags — [Westlaw], [web search — verify],
  [model knowledge — verify]. That's playbook §0.5 and §0.6 enforced at the skill level.

- Distinguishes gap types (a missing DPA and an unread DPA are labelled differently),
  names its approver rather than "escalate to legal", and includes a jurisdiction delta
  check when the playbook's governing-law preference conflicts with the contract's.

Fragment note: this skill is `user-invocable: false` and is loaded by /commercial-legal:review
in its origin plugin. Under playbook §4.8's "Wraps / Custom + Marketplace" case, the wrapping
is done by scribe's custom skill `contract-review-routing` (not yet built), which detects
agreement type and rebinds the config path
`~/.claude/plugins/config/claude-for-legal/commercial-legal/CLAUDE.md` onto YVON's
`Teams/Legal & Compliance/scribe/operational/agent/scribe-config.md`. The marketplace
copy below remains unaltered.

The SMB skill's 8-category contract-risk checklist is cited as a source for scribe's future
custom `contract-library` registry skill; not merged (a merge would collapse both to
custom per §4.6 and forfeit the verbatim-marketplace benefit).
-->
