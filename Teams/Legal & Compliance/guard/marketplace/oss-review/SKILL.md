...
source_raw: https://raw.githubusercontent.com/anthropics/claude-for-legal/main/ip-legal/skills/oss-review/SKILL.md
source_repo: anthropics/claude-for-legal
author: anthropics (Anthropic PBC)
fulfills_catalog_entry: oss-policy (Legal & Compliance · guard · IP Protection)
...
Selected 2026-07-29 as guard's OSS license compliance skill.

Why:
- Deployment-model-first analysis — the same license carries different obligations
  depending on SaaS / binary / internal / embedded. Kills the "one-size" answer.
- Explicit linking-relationship analysis (static / dynamic / IPC / network / MPL file-scope).
  Static-linked LGPL in proprietary product = Critical; dynamic-linked = Low. Same license,
  opposite rating. Nuance most OSS tools miss.
- Non-OSI source-available flagging (SSPL, BUSL, Commons Clause, Elastic License, fair-source) —
  won't call them "open source" just because GitHub says so.
- License-unknown = "needs review", never permissive-by-default. Ship-decision-blocking.
- Every citation source-tagged; contested questions (AGPL network trigger, GPL-3.0 conveying)
  flagged for attorney rather than fabricated.

Merge candidate deferred: `erp-mafia/oss-license-compliance` (185⭐, SPDX/ScanCode/AGPL §13 /
SSPL / BSL depth + NIST-800-53/ISO-27001/SOC 2 mappings) has more corporate-compliance depth
but merging = collapses to custom per §4.6. Deferred.
-->
