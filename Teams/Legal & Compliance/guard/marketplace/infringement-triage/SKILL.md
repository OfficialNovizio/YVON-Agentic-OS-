...
source: https://github.com/anthropics/claude-for-legal/tree/main/ip-legal/skills/infringement-triage
source_raw: https://raw.githubusercontent.com/anthropics/claude-for-legal/main/ip-legal/skills/infringement-triage/SKILL.md
source_repo: anthropics/claude-for-legal
author: anthropics (Anthropic PBC)
fulfills_catalog_entry: infringement-triage (added — not in original catalog; broadens guard's marketplace coverage)
...
Selected 2026-07-29 as guard's cross-right infringement triage skill.

Why:
- Covers all four IP rights (TM / copyright / patent / trade secret) in one
  skill with mode selection — refuses to blend rights that have different
  factors / defenses / remedies.
- "This is a triage, not a finding" — loudest guardrail; matches §0.5/§0.7.
- Design patent branch is separate (ordinary-observer test, §289 total-profits
  damages, trade-dress cross-flag) — catches the common error of applying
  utility-patent claim charts to D-numbers.
- Jurisdiction-specific analysis (US claim chart vs Germany/China utility
  models vs UPC) — refuses to silently apply US framework internationally.
- Explicit handoff protocol to `cease-desist` / `takedown` — the decision to
  assert is the approver's, not the triage's.

Not in original catalog — added to broaden guard's marketplace coverage since
this fills a real gap between clearance (pre-adoption) and cease-desist
(post-decision).
-->
...
Use the applicable circuit's multi-factor test (du Pont / Polaroid / Sleekcraft / other — see the `clearance` skill for the case citations and pick logic). Walk each factor and flag what cuts each way — similarity of marks (sight / sound / meaning / commercial impression), similarity of goods or services (expected-source test), channels of trade, consumer sophistication, strength of senior mark (fanciful / arbitrary / suggestive / descriptive / generic), intent (evidence of copying), actual confusion, likelihood of expansion.

Dilution: apply federal TDRA (15 U.S.C. § 1125(c)) — fame threshold requires fame to the general consuming public (*Starbucks Corp. v. Wolfe's Borough Coffee*, 588 F.3d 97 (2d Cir. 2009)). Blurring vs tarnishment; comparative-advertising / news-reporting / fair-use / non-commercial defenses.

False advertising / comparative claims: Lanham Act § 43(a) — materiality, falsity-or-misleading, deception, commercial-speech, injury. Flag literally false vs implicitly false vs puffery.

Output: factors table + what cuts each way + a "not a finding" conclusion line + routing per enforcement posture.

---

## Copyright mode

**Ownership** — claimant is owner (or exclusive licensee with standing); work-for-hire / joint authorship / assignments / termination rights all flag.

**Registration** — 17 U.S.C. § 411 requires actual registration (or preregistration) before filing (*Fourth Estate Public Benefit Corp. v. Wall-Street.com*, 586 U.S. 296 (2019)); flag if not registered.

**Access + substantial similarity** — two paths: (a) access + probative similarity; (b) striking similarity supporting inference despite absent proof of access. For substantial similarity, apply the circuit's test (Second Circuit ordinary-observer; Ninth Circuit extrinsic/intrinsic under *Krofft* / *Swirsky*; Fourth / Seventh / Eleventh variations).

**Fair use** — 17 U.S.C. § 107 four factors: purpose and character (transformativeness; commercial); nature of work (factual vs creative); amount and substantiality; effect on market. Recent touchstones: *Google LLC v. Oracle America*, 593 U.S. 1 (2021); *Andy Warhol Found. v. Goldsmith*, 598 U.S. 508 (2023) — flag transformativeness carefully; *Warhol* narrowed.

**DMCA safe harbor** — 17 U.S.C. § 512. If accused is a service provider hosting user content: § 512(c) — designated agent, notice-and-takedown, no actual or red-flag knowledge, no financial benefit attributable to controllable infringement, expeditious takedown, repeat-infringer policy. Safe harbor does NOT cover direct infringement by the service provider itself.

Output: factors flagged, fair-use balance without concluding, ownership / registration / safe-harbor threshold notes, routing per posture.

---

## Patent mode

**Route to `/ip-legal:fto-triage` for the detailed framework.** This mode is the mirror image of the FTO skill — same claim charts, same DOE flag, same all-elements rule — applied to an accused product instead of one's own.

### Design patent (D-number) — branch before the workflow

**Check the asserted patent's registration number FIRST.** If it has a `D`, `RE`, or `PP` prefix (e.g., `D712,345`), it's not a utility patent and the workflow below does NOT apply. Branch per prefix:
...
- **Ask for the drawings.** "I can't run the ordinary-observer test without seeing the patent figures and the accused product. Paste or attach: (a) the patent drawings (all figures, including any broken-line disclaimers), (b) photos of the accused product from comparable angles, (c) any prior art designs you're aware of."
- **Prior-art landscape.** Ordinary observer is a *comparison* test — the observer is "familiar with the prior art," so the scope of the patented design narrows as the prior-art field crowds. Flag what prior art is known and what's missing.
...
- **Broken lines.** Design patents use solid lines for claimed features and broken lines for unclaimed environmental context. Flag whether the alleged copying is in claimed (solid-line) or unclaimed (broken-line) territory.
...
The rest of this mode assumes the asserted patent is a **utility patent** (no `D`/`RE`/`PP` prefix). If the D-number branch above applies, stop here.
...
- **Damages posture** — lost profits vs. reasonable royalty (Georgia-Pacific factors), marking, pre-suit notice, willfulness (reading this triage may factor into willfulness — see the FTO skill's willfulness note).

### Output

Claim charts. Element flags. Defense flags. Routing to patent counsel. See the `fto-triage` skill for the full output structure — the infringement-triage patent mode uses the same format with "accused product" substituted for "own product."

### Handoff to the full claim chart

For a detailed element-by-element claim chart suitable for infringement or invalidity contentions, run `/litigation-legal:claim-chart`. This triage's claim chart is a first pass to identify the strongest and weakest mappings; the litigation claim chart builds the full chart with pin cites, claim construction flags, dependent claims, and the verification workflow that contentions require.
...
Apply DTSA (18 U.S.C. § 1836 et seq.) federally and applicable state UTSA (or common-law tests in New York / Massachusetts / other non-UTSA jurisdictions). Flag:

- **Not generally known** — to the public or others in the industry who can obtain economic value from disclosure.
- **Economic value from secrecy** — independent economic value actual or potential, derived from not being generally known.
- **Combinations and compilations** — a combination of public elements can be a trade secret (*Altavion v. Konica Minolta*; Restatement view).
...
Flag what's in place and what's missing. *Reasonable* is fact-specific; the triage lists what's in place, does not decide reasonableness.
...
- **Inadvertent disclosure:** disclosure by person with a duty? Recipient know or have reason to know?
- **Reverse engineering** — a defense if the means were lawful. Flag whether reverse engineering is plausible on the facts.

### Preemption

Where state tort claims (unfair competition, conversion, breach of confidence) might be preempted by UTSA, flag preemption. Some jurisdictions preserve contract claims; others preempt most tort claims addressing the same facts.
...
> I'll use the flag list from this triage as the factual basis and apply the approval chain from your practice profile — the letter won't go anywhere without the approver signing off.
...
- **Draft the C&D, takedown, or complaint.** Those are separate skills (`/ip-legal:cease-desist`, `/ip-legal:takedown`) gated by the approval chain in the practice profile.
...
Factor-by-factor, flag-by-flag. No hedging prose. The guardrail at the top does the scope work; the analysis does the analysis. A lawyer should leave the output knowing exactly which factors are flagged, which defenses apply, and what they need to do next to either assert or stand down.
