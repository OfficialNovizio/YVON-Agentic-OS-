...
source: https://github.com/anthropics/claude-for-legal/tree/main/regulatory-legal/skills/reg-feed-watcher
source_raw: https://raw.githubusercontent.com/anthropics/claude-for-legal/main/regulatory-legal/skills/reg-feed-watcher/SKILL.md
source_repo: anthropics/claude-for-legal
author: anthropics (Anthropic PBC)
fulfills_catalog_entry: reg-feed / compliance-matrix (Legal & Compliance · comply · Compliance Lead)
assigned_agent: comply (Legal & Compliance / Compliance Lead — department leader)
portable: false            # hardcoded plugin config path + Federal Register API bias; wrapped by comply/custom/reg-monitor-routing (to be built) which rebinds config onto operational/agent/comply-config.md and lets watchlist/feeds be non-US
date_added: 2026-07-29

# --- YVON compile metadata (playbook §14.3, §14.4) ---
tier: 3                    # config-dependent (materiality threshold, watchlist, feed configuration all live in config)
triggers:
  - check the feeds
  - what's new
  - regulatory update
...
Selected 2026-07-29 as comply's regulatory-monitoring marketplace skill.

Why:
- Config-driven — reads watchlist, materiality threshold, feed configuration from
  a config file, matches YVON's operational/agent/<agent>-config.md pattern (§7, §14.7).
- Explicit "no silent supplement" gate — if feeds return thin, the skill asks the
  operator to choose (broaden window, different feed, web search with [verify] tag,
  or stop) rather than filling gaps from model knowledge. Same discipline as
  scribe's vendor-agreement-review.
- Every citation and item carries a source-provenance tag ([Federal Register],
  [regulator RSS], [CourtListener], [web search — verify], [model knowledge — verify],
  [secondary source]). Enforces playbook §0.5 and §0.6 at the skill level.
- Secondary-source handling — IAPP, FPF, Lexology, etc. are tagged distinctly and
  bumped down a tier until traced to primary. Kills the "reg-blogger said so"
  fabrication vector.

Fragment note: like scribe's vendor-agreement-review, this skill loads its config
from `~/.claude/plugins/config/claude-for-legal/regulatory-legal/CLAUDE.md`, a path
that does not exist in YVON. Under playbook §4.8's "Wraps / Custom + Marketplace"
case, a companion custom skill in comply — planned as `reg-monitor-routing` —
detects the intent, rebinds this config path onto comply's
`operational/agent/comply-config.md`, and lets the watchlist be non-US
(playbook §0.4b — no venture/jurisdiction hardcoding).

Portability caveat: the skill body assumes US Federal Register API + US agency
slugs. The wrapper is what makes it jurisdiction-neutral for YVON's genericisation
discipline. This marketplace copy remains verbatim.
-->
