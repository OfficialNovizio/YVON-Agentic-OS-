...
| agent-reach | ✅ `/opt/yvon-tools/venvs/agent-reach` + `/usr/local/bin` + skill (**VPS**) | ⚠ verify | Read Web/YouTube/GitHub/RSS/V2EX (+opt-in Twitter/Reddit) | cypher, meta |
...
| opensandbox (SDK/CLI/MCP) | ○ (VPS, needs Docker) | ⚠ verify | Isolation runtime — quarantine box (§7.7) | ops, warden, bastion, dana, scout; mia, quinn, raj, nova (MCP) |
...
| **turbovec** | ✅ VPS venv `/opt/yvon-tools/venvs/turbovec` (import OK) | MIT | **Graph-brain episodic engine**: fuzzy vector recall (TurboQuant, ~4GB/10M, online ingest, **search-time allowlist = brain isolation**, O(1) remove = forgetting). Complements graphify. Not in Shared OS/tools — VPS venv | dana, rank, scout, quinn (episodic recall) |
| **fastembed** | ✅ VPS venv `/opt/yvon-tools/venvs/turbovec` (with turbovec) | Apache-2.0 | Local ONNX text embedder (no PyTorch) — turns memory text → vectors that feed turbovec. The air-gapped half of the episodic layer | (companion to turbovec) |
