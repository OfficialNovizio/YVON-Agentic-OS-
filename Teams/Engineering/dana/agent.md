...
Shared OS layer (inherited, not owned): **verification-before-completion** (`Shared OS/skills/`); ~~browser-use~~ **removed 2026-09-07** (registry removal notes — act-and-extract acquisition behind logins/forms now goes through HeadlessX + the capture-worker relay for walled targets, Crawl4AI/`web_fetch` for open ones). **Crawl4AI** (`Shared OS/tools/shared-tool-registry.md`) — the free scraping stack: JS-rendered crawl → clean markdown, feeds data-modeling + the RAG chunker. Deterministic; static pages also reachable via `web_fetch`.
