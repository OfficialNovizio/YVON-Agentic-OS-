...
| exploratory-qa | ~~Shared OS tool: browser-use~~ — **removed 2026-09-07**, zero callers (registry removal notes) | Exploratory QA now runs through the **browser-verification** pair above: Reticle-gated interactive exploration + the Playwright suites; every bug it finds becomes a scripted Playwright regression (webapp-testing) so it's gated forever. If an NL-driven agent is ever re-wanted, it must re-pass `quarantine.sh`. |
