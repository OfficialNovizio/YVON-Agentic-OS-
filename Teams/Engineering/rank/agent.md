...
| browser-audit | ~~Shared OS tool: browser-use~~ — **removed 2026-09-07**, zero callers (registry removal notes) | Rendered-page auditing where a crawler stops short (JS-gated content, interactive states, multi-step journeys) now goes through **HeadlessX** (stealth render/crawl/map) + the capture-worker relay for walled targets. Recommends, never auto-edits (same boundary as claude-seo-integration). |
