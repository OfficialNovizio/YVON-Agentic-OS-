...
> (2026-08-10). The prompt craft below is provider-independent and unchanged from
> upstream; only the CLI-flag call-outs (Higgsfield-specific `--mode`/`--resolution`
> syntax) were removed and replaced with pointers to `SKILL.md`'s krea.ai model roster
> (Step 4), since krea's parameters differ by route.
...
- `MOBILE` — yes/no. **Always asked** (SKILL Step 1.6), presented to the user as **beta**. Gates the `-m.mp4` encodes (pipeline.md §6b) + `clipMobile`/`connectorsMobile` wiring + the full mobile QA.
...
- Aspect `3:2`. Model + resolution: SKILL.md Step 2 (`openai/gpt-image-2` default for
  finals, `bfl/flux-1-dev` for cheap draft/previz).

## Leg prompt — architecture A, continuous forward take (Step 4)

`start_image = previous leg's ACTUAL last frame` (leg 0: the first scene's still).
...
`start_image = the scene still` (solid-bg version).
...
Duration/aspect: `16:9`, `duration 8` for dives (SKILL.md Step 4 roster table — confirm
against `docs.krea.ai` before batching; params vary per krea route).

## Connector clip prompt (Step 5)

`start_image = dive_i LAST frame` (extracted), `end_image = dive_{i+1} FIRST frame`
...
Duration/aspect: `16:9`, `duration 5` for connectors. Connectors need an end-image, so
the model must accept one — see SKILL.md Step 4's roster and its qualification-probe
requirement before relying on this for a real batch.
