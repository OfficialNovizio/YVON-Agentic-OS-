...
source: https://skillsmp.com/creators/borghei/claude-skills/project-management-execution-north-star-metric
source_repo: https://github.com/borghei/Claude-Skills/tree/main/project-management/execution/north-star-metric
author: borghei
license: MIT + Commons Clause (from source frontmatter, v1.0.1, updated 2026-06-15)
copied_verbatim: true
fulfills_catalog_entry: vyon-north-star-metrics (VYON_Skills_Catalog_Full_v2.html, vista/Executive Office) — catalog listed this as custom; fulfilled by this marketplace skill (guardrails/thresholds = the catalog's "3 guardrails + alert thresholds"), with the catalog's per-venture tracking + monthly-review-with-marcus cadence deferred to vista's operational layer per 2026-07-06 discussion
assigned_agent: vista (Executive Office / Roadmap Lead)
portable: true — no company-specific content; original catalog entry's hardcoded venture NSMs (per rule 0.4b) are NOT carried over; every NSM is defined per business at runtime
...
<!--
  Unmodified copy of the "north-star-metric" skill, selected to fulfill the catalog's
  "vyon-north-star-metrics" slot for vista. Chosen over phuryn/pm-skills' north-star-metric (22k★)
  because this version carries the half the catalog entry actually requires: anti-metrics and
  counter-metrics WITH explicit thresholds (= the catalog's "3 guardrails each; alert thresholds"),
  plus the metric_tree_builder.py rendering tool. The phuryn version defines NSM + inputs only.
  The catalog entry's remaining elements — "confirm NSM per venture" and "monthly review with
  marcus" — are workflow cadence, not metric methodology, and will live in vista's operational/
  layer (commands + skill routing) when built.
  NOTE: the source's Integration Points table references its home repo's other skills
  (brainstorm-okrs, outcome-roadmap, etc.). Kept verbatim; in this system the equivalents are:
  brainstorm-okrs → marcus/okr-cascade; prioritization-frameworks → vista/rice-prioritization;
  OKR verification → vista/okr-quality-checker.
  Script tested 2026-07-06: `--demo --format markdown` and `--format mermaid` both run clean
