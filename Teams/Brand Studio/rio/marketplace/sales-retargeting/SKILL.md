...
type: marketplace
status: copied verbatim (SKILL.md; source's references/platform-guide.md remains in the source repo — see pending note)
source: https://skillsmp.com/creators/sales-skills/sales/skills-sales-retargeting
source_repo: https://github.com/sales-skills/sales/tree/main/skills/sales-retargeting
author: sales-skills
license: MIT (from source frontmatter, v1.0.0)
copied_verbatim: true
fulfills_catalog_entry: retargeting-playbooks (VYON_Skills_Catalog_Full_v2.html, rio/Brand Studio)
...
<!--
  Unmodified copy of "sales-retargeting" (sales-skills/sales, 64★), fulfilling the catalog's
  retargeting-playbooks slot ("audience segmentation and sequential messaging patterns;
  frequency caps"). Chosen for its honesty: the Gotchas section carries exactly the caveats
  this system enforces elsewhere — view-through ROAS inflation, retargeting cannibalizing
  organic (true lift 40–60% of reported), iOS/cookie erosion — plus a self-improving
  learnings.md convention that matches our self-annealing rule.
  NOTES FOR THIS SYSTEM:
  1. The source's /sales-{platform} skill ecosystem and its cross-skill fetch protocol are
     source-repo machinery — read those sections as pointers; platform-specific setup here
     routes to rio's own ad-platform playbooks and the configured connectors.
  2. Email-retargeting strategy overlaps lena's email-marketer: strategy said here, sequence
     design and sending discipline are lena's.
  3. Budget-allocation suggestions (40/30/20) are the source's starting shapes — actual
     allocations live in rio-config and board's budget envelope.
-->
