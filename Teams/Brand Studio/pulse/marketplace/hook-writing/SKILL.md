...
source: https://skillsmp.com/creators/revfactory/harness-100/en-01-youtube-production-claude-skills-hook-writing
source_repo: https://github.com/revfactory/harness-100/tree/main/en/01-youtube-production/.claude/skills/hook-writing
author: revfactory
copied_verbatim: true
fulfills_catalog_entry: none — pulse is the new v3 agent; this fulfills its platform-native-formats slot's DURABLE layer per the 2026-07-07 three-layer hook design
...
<!--
  Unmodified copy of "hook-writing" (revfactory/harness-100, 1.1k★ — same trusted repo as
  board's okr-quality-checker). This is the STATIC layer of pulse's three-layer hook design:
  the 15 patterns and their psychological mechanisms (curiosity gap, loss aversion,
  self-reference, narrative tension) are stable human attention mechanics.
  THE OTHER TWO LAYERS LIVE ELSEWHERE, BY DESIGN:
  - volatile platform executions → the dated, refreshable platform playbooks
    (social-content-calendar/assets/platform-playbook-template.md)
  - ground truth → the hooks register (what actually worked for THIS audience, fed by kai)
  The register OUTRANKS these patterns; patterns fill gaps and are flagged as craft
  heuristics per rule 0.6.
  FRAMING NOTE: the source is YouTube-framed (3-second decision, retention benchmarks,
  hook–thumbnail–title triangle). The psychology transfers to every feed — "first 5 seconds"
  maps to first line (X/LinkedIn), first frame (TikTok/Reels), cover+title (YouTube) — with
  the per-platform mapping owned by each playbook's §3. The retention benchmark table is
  YouTube-specific illustration, not a cross-platform standard.
  HONESTY LAW: pattern #1/#12-style stats and authority quotes are TEMPLATES — real numbers
  and real sources only, per the system-wide no-fabrication rule. The source's own
  anti-clickbait rule (hook must deliver the promise) is binding.
-->
