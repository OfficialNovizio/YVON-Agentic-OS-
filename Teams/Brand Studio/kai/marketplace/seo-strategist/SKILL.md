...
source: https://skillsmp.com/creators/keith-gjino/marketing-skills/seo-strategist
source_repo: https://github.com/KEITH-GJINO/Marketing-Skills/tree/main/seo-strategist
author: KEITH-GJINO
copied_verbatim: true
fulfills_catalog_entry: seo-audit (VYON_Skills_Catalog_Full_v2.html, kai/Brand Studio)
...
<!--
  Unmodified copy of "seo-strategist" (KEITH-GJINO/Marketing-Skills — same trusted repo as
  lena's email-marketer), fulfilling the catalog's seo-audit slot ("technical + content SEO
  review: crawl issues, content gaps vs intent, prioritized fix list"). This version adds
  the diagnostic ladder ("why isn't my page ranking" — indexed → intent → authority →
  links → quality → technical), topic-cluster strategy, the brief structure, and honest
  push-back rules (no keyword stuffing, no black-hat, 3–6 month expectations).
  NOTES FOR THIS SYSTEM:
  1. Audience/positioning context loads from kai's brand-context file; content briefs this
     skill produces hand to lena (writing) and the calendar (pulse) — kai aims, others write.
  2. SEO movement reports into marketing-dashboards §6.
  3. Keyword/SERP data arrives via configured tools or operator-supplied exports; the shared
     web-search layer extends this when built.
-->
