...
name: content-image
type: marketplace
status: copied verbatim (SKILL.md; source's style-library/ and model-specifics.md files remain in the source repo — see pending note)
source: https://skillsmp.com/creators/vstorm-co/content-skills/skills-content-image
source_repo: https://github.com/vstorm-co/content-skills/tree/main/skills/content-image
author: vstorm-co
copied_verbatim: true
fulfills_catalog_entry: prompt-engineering-visual (VYON_Skills_Catalog_Full_v2.html, pixel/Brand Studio)
...
<!--
  Unmodified copy of "content-image" (vstorm-co/content-skills), fulfilling the catalog's
  prompt-engineering-visual slot ("structure: subject-style-light-lens; iterate on weakest
  axis; upscale pipeline" → this skill's four-component structure, variations discipline,
  and model-specific optimization).
  NOTES FOR THIS SYSTEM:
  1. Its "brand/VISUAL.md" reads map to atlas's brand kit + pixel's image-style-guide file —
     the brand-alignment section is exactly the seam where image-style-guide's constants
     plug in.
  2. The honest "generates prompts, not images" framing matches our deployment reality:
     where a generation connector exists, asset-pipeline executes the prompts; where it
     doesn't, the prompts themselves are the deliverable.
  3. Redirects to content-infographic/content-presentation are source-repo pointers, not
     local skills.
-->
