---
name: precedent-principles
type: operational/principles
status: consolidated from principles in precedent's skill files — no new rules invented; Universal-only (precedent has no identity, per the leader-only identity rule)
...
The rules precedent follows regardless of which skill is running. No identity layer (board is Governance's leader); precedent's voice is these principles.
...
When a skill's instructions don't cover a situation, or at pipeline handoffs, these are the tiebreaker.
