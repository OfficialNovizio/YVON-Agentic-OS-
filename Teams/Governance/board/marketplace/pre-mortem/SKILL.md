...
source: https://skillsmp.com/creators/guia-matthieu/clawfu-skills/skills-strategy-pre-mortem
source_repo: https://github.com/guia-matthieu/clawfu-skills/tree/main/skills/strategy/pre-mortem
author: guia-matthieu (ClawFu / MKTG Skills)
license: MIT (from source frontmatter, v1.0.0)
copied_verbatim: true
fulfills_catalog_entry: pre-mortem (VYON_Skills_Catalog_Full_v2.html, board/Governance)
...
<!--
  Unmodified copy of the "pre-mortem" skill, selected to fulfill the catalog's pre-mortem slot
  for board — prospective failure analysis before major commitments. Chosen over phuryn/pm-skills'
  pre-mortem (22k★, Tigers/Paper Tigers/Elephants) because that version is built around PRODUCT
  LAUNCHES (14-day launch framing, launch-blocking classification), while this one implements
  Gary Klein's original prospective-hindsight method explicitly scoped to DECISIONS — hiring,
  investment, partnership, resource commitments — which is what a governance gate reviews.
  Matches the catalog protocol directly: assume failure at a future date → generate causes →
  triage (likelihood × impact × detectability) → mitigate top risks before approval → tripwires.
  KNOWN SOURCE DEFECT, kept verbatim per copy-without-cutting rule: the "What Claude Does vs
  What You Decide" table and the "Skill Boundaries" section contain copy-paste text from an
  unrelated AUDIO-PRODUCTION skill in the same source repo ("structuring audio production
  workflows," "script outlines," etc.). Ignore those two sections when executing — every other
  section is correct and internally consistent. Flagged here rather than silently edited.
  BOUNDARY NOTE for this system: marcus's decision-critic red-teams a proposal to IMPROVE it
  before it is proposed; board's pre-mortem is part of the GATE that rules on it. A decision
  that passed decision-critic still goes through board's gate.
-->
