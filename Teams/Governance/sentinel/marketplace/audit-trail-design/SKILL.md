...
source: https://skillsmp.com/creators/dtsong/my-claude-setup/skills-council-guardian-audit-trail-design
source_repo: https://github.com/dtsong/my-claude-setup/tree/main/skills/council/guardian/audit-trail-design
author: dtsong
copied_verbatim: true
fulfills_catalog_entry: audit-trail-method (VYON_Skills_Catalog_Full_v2.html, sentinel/Governance)
...
<!--
  Unmodified copy of "audit-trail-design", selected to fulfill the catalog's "audit-trail-method"
  slot — standard audit logging and chain-of-custody practices. Matches the catalog protocol
  directly: log actor/action/timestamp/basis (its who/what/when/where/why schema), immutable
  append-only (WORM, hash chaining, separation of duties), periodic integrity checks. Chosen over
  marquesfelip/audit-trail-design (thinner) and domain-bound variants (healthcare/HIPAA-specific).
  NOTES FOR THIS SYSTEM:
  1. The source's Handoff section references its home repo's skills (compliance-review,
     data-classification). Kept verbatim; in this system the compliance-review equivalent is
     sentinel's own future compliance scope + the operator's counsel, and data classification
     is not yet assigned to any agent.
  2. The regulatory retention figures (HIPAA/SOC2/PCI-DSS) illustrate the method. Actual
     retention obligations per business and jurisdiction (e.g., Canadian requirements like
     CRA's 6-year books-and-records rule) are operator-supplied inputs, not this file's claims.
  3. This skill DESIGNS audit trails; sentinel's constitution-watch and gate-bypass-detection
     are consumers of trails designed this way, and precedent's decision log follows its
