...
source: https://github.com/anthropics/knowledge-work-plugins/tree/main/small-business/skills/cash-flow-snapshot
source_raw: https://raw.githubusercontent.com/anthropics/knowledge-work-plugins/main/small-business/skills/cash-flow-snapshot/SKILL.md
source_repo: anthropics/knowledge-work-plugins
author: anthropics (Anthropic PBC)
fulfills_catalog_entry: runway-model / cash-flow-forecast (Finance & Treasury · felix · Finance Lead)
assigned_agent: felix (Finance & Treasury / Finance Lead — department leader)
portable: true            # method-only, no plugin config path assumed
...
Selected 2026-07-29 as felix's cash-flow marketplace skill.

Why:
- Method-driven with real math (payment lag mean + variance → confidence bands).
- Multi-source (QB, Stripe, PayPal, Square, CSV) with named fallback discipline.
- Explicit "no auto-fix / no destructive" gate — read-only.
- Confidence bands honestly capped at ±50% with "flag it instead" — anti-fake-precision.
- Portable: no plugin config path assumed.
- Runs the operator's actual data through documented formulas — matches YVON's
  §0.5 no-invention discipline.

Note: the skill assumes an owner-role framing (small-business owner). felix's
custom skills provide the org/CFO framing on top; this marketplace skill provides
the compute engine.
-->
