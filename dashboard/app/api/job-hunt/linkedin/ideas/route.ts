/**
 * /api/job-hunt/linkedin/ideas — post idea bank (Content Lab).
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    let query = sb.from('post_ideas').select('*').order('created_at', { ascending: false })
  ...
  try {
    ...
    const { data, error } = await sb
      ...
      .insert({
        ...
        expanded_draft: body.expanded_draft ?? null,
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    const sb = getServiceClient()
    const { data, error } = await sb.from('post_ideas').update(updates).eq('id', id as string).select().single()
    if (error) return Response.json({ error: error.message }, { status: 500 })
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    const sb = getServiceClient()
    const { error } = await sb.from('post_ideas').delete().eq('id', id)
    if (error) return Response.json({ error: error.message }, { status: 500 })
