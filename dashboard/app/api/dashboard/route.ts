...
export async function GET(request: Request): Promise<Response> {
  try {
    ...
    // Fleet baseline — the real org from Teams/ (grows as agents/departments are added).
    // When no live sessions are reported, show the true fleet size instead of 0.
    if (!supabaseConnected || agentsLive === 0) agentsLive = FLEET.length
    ...
    const data = {
      ...
      activity,
      ventures,
      // Real fleet from Teams/ (source of truth) — grows automatically as agents/departments are added.
      fleet: {
        totalAgents: FLEET.length,
        departments: FLEET_DEPARTMENTS.length,
        byDepartment: FLEET_DEPARTMENTS.map((d) => ({ department: d, count: fleetByDepartment(d).length })),
      },
    }
