...
**0.9 — Compact presentation format (added 2026-07-29).** During discovery, roster review, source presentation, and any operator-facing message where the point is a decision or a plan: deliver information as tables (typically: agent · skill · type · one-line rationale · URL), not prose paragraphs. Long-form explanation belongs inside SKILL.md files, identity documents, persona files, and operational subfolder content — *not* in operator-facing chat. The operator reads a table faster than three paragraphs and can correct a table in place. This rule is senior to the natural pull toward exhaustive explanation. When the operator explicitly asks for reasoning or context, expand; by default, compress.

**0.10 — Every UI is mobile + tablet responsive from the FIRST commit (added 2026-07-30).** Not "we'll polish later." A page/component/modal that only works on a wide desktop is a bug, not "unfinished." The three breakpoints:
...
**1.2 — Pick the agent within that department.** List the agents in that department (from the org chart / catalog) as a compact table (agent · role · leader?), **recommend one to start with (usually the department leader)**, and ask the operator to confirm or pick another. Present per §0.9.
...
**4.3 — Present sources before touching anything (apply rule 0.1 here explicitly).** For each candidate, give the person: what it is (the skill's actual description), why it fits (the reasoning, compared against alternatives if any), and its source URL. **When multiple marketplace candidates fit the same purpose, present all of them in one table (name · source URL · one-line rationale) with the recommended pick marked ★.** Per §0.9, prefer the table over prose. Wait for explicit approval before doing anything else.
...
**8.7 — Where extracted artifacts live (updated 2026-07-14).** The output of logical extraction is a Python script. All logical .py scripts live in `Shared OS/logical/` (see §13.5). No .md reference files are kept in logical/ folders; the only .md file in any agent's `logical/` is `book-requirements.md`. Agents import from `Shared OS/logical/` — they never copy scripts. Skills call the scripts directly by path (e.g., `Shared OS/logical/capital_budgeting.py`). Each `book-requirements.md` is updated from "placeholder" to "sourced from [book], script → Shared OS/logical/<script>.py."
	...
	**8.14 — End-of-agent book step (added 2026-07-29).** After the operational subfolders are done and *before* `agent.md` is finalised, present the book type(s) this agent needs — Route A (math), Route B (rule), Route C (hybrid), Route D (practitioner wisdom) per §8.2 — with 2–3 candidate books per Route. Present as a table per §0.9 (Route · book · author + credential · why this book · free-source status). The operator picks a candidate or names their own; if the operator names a book not in the list, dig into it and treat it as the pick. Then run §8.8a's 3-attempt free-source hunt for whichever books were selected; present §8.8b's structured research summary; the operator chooses the path (build now from free sources, source paywalled books first, or defer). This is the single named step where books enter the pipeline — it does not replace §8.1's two-touch discipline (touch 1 = placeholder now, touch 2 = extraction later), it makes the "propose books" moment explicit in the build order (see §12 step 8b).
...
1. Department → 2. Agent (§1.2 — recommend one, table format per §0.9) → 3. Discuss skill list (table: skill · type · one-line rationale · URL) → 4. Sort marketplace/custom, **default to marketplace first** → 5. Marketplace skills (search by purpose, present candidates as one table with ★ recommendation per §4.3, **stop**, then copy, one at a time) → 6. Custom skills (discuss design + tools/scripts, build one at a time) → 7. Identity (leader agent only, one persona) → 8. Operational — **check which of the five subfolders actually apply to this agent, prioritize the most load-bearing one first, skip any that would come out empty**, build one at a time, each with a pre-build discussion → **8b. Propose books (§8.14): table of Route × candidate books; operator picks or names own; run 3-attempt free-source hunt (§8.8a); present research summary (§8.8b); operator chooses path** → 9. Logical **placeholder** (record the flagged 0.6 judgments + selected book — see §8's two-touch note) → 10. agent.md (kept current throughout) → 11. Department workflow file (only once the whole department is done) → **12. Compile + reindex (§14): `node cli/skillgen.js <agent>` — zero FILL_INs — then `cd rag && python3 core/chunkify.py --all`; add the routing row to root `CLAUDE.md` §2** → repeat for the next agent.
...
**13.5 — Logical scripts: cross-agent vs single-agent placement (added 2026-07-14, refined 2026-07-29).** Logical .py scripts belong in one of two places, decided by scope, not by convenience:

  - **Cross-agent (default expectation) → `Shared OS/logical/`.** A source book grounds ~10 agents (§8), so most logical scripts are cross-agent from the start. `Shared OS/logical/` is a single flat folder — no subfolders, no per-agent subfolders. Skills import by absolute path (e.g., `Shared OS/logical/capital_budgeting.py`); the script is never copied into a consuming agent's folder (§13.1).
  - **Genuinely single-agent → `custom/<skill>/scripts/`.** A logical script that only one agent will ever use may live under that agent's `custom/<skill>/scripts/`, provided it meets the same discipline: real source book with whole-book access (§8.10), minimum-two-book grounding (§8.0), honest docstring citing chapter/page, and self-tests (§5.2). Reasoning-based defaults (thresholds, scales) are §0.6-flagged in the docstring, exactly the same way `Shared OS/logical/` scripts flag them.
  - **Promotion rule — if a second agent needs it, migrate first.** A `custom/<skill>/scripts/` script that a second agent later wants to import must be moved to `Shared OS/logical/` *before* the second agent adopts it. Copy-paste across agents is forbidden (§13.1). The move triggers a recompile + reindex + retoonify of both agents.
  - **Anti-fake-Tier-C rule (added 2026-07-29).** A script whose logic is invented from general knowledge — no source book, no canonical standard (§8.4 Tier B), no operator-supplied config — does not belong in *either* location. §0.5 forbids the invention; §8 forbids putting invented content in the logical layer; §8.4 Tier C means *stays reasoning-based, explicitly — not faked into a script.* If the skill needs the functionality before touch-2 lands, the skill's Step description does the extraction as LLM-based reasoning with the `[REASONING-BASED]` flag, and the script is deferred to touch-2. The failure mode this closes: writing a script with plausible-looking patterns from general knowledge and shipping it, which passes casual inspection but grounds the agent's judgments in nothing.
  - **Agent `logical/` folders remain minimal.** Each agent's `logical/` folder contains ONLY `book-requirements.md`. No .py files. This is unchanged.
  - **Books produce scripts or cited .md files.** The synthesis pipeline (§8.3) produces Python scripts with self-tests for anything with formulas (Routes A/B/C). Route D (practitioner-operator wisdom, §8.9) may produce a cited .md file instead — qualitative extracts with chapter/page references. Docstrings carry book/section/page citations.
  - **`Shared OS/logical/` is the system of record for cross-agent scripts.** Updates propagate to every consuming agent automatically — §8.7's "extract once, cite many" enforced at the file level.
