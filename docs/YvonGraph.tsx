...
/* ═══════════════════════════════════════════════════════════════════════
   ...
   ⚠ DATA BELOW IS PLACEHOLDER — I do not have your Graphify export.
     Replace DEPARTMENTS[] with the real structure. Shape needed:
       { id, name, metric, metricLabel, agents: [{ id, name, tag }] }
   ═══════════════════════════════════════════════════════════════════════ */
...
function d(id: string, name: string, metric: string, metricLabel: string, agents: string[]): Dept {
  return {
    id, name, metric, metricLabel,
    agents: agents.map((a, i) => ({
      id: `${id}-${i}`,
      name: a,
      tag: ["Data", "Creative", "Ops", "Review"][i % 4],
    })),
  };
}

const DEPARTMENTS: Dept[] = [
  d("exec",     "Executive",        "12", "Decisions Pending",  ["Chair","Ops Lead","Chief of Staff","Delegate","Board Sync","Escalation","Policy","Charter"]),
  d("strategy", "Strategy",         "07", "Active Initiatives", ["Roadmap","OKR","Market Read","Scenario"]),
  d("finance",  "Finance",          "24", "Open Ledgers",       ["Ledger","Forecast","Burn","Treasury","Audit"]),
  d("legal",    "Legal",            "03", "Contracts in Review",["Contract","Compliance","IP"]),
  d("people",   "People",           "18", "Open Roles",         ["Recruiting","Onboarding","Review","Culture"]),
  d("product",  "Product",          "42", "Specs Active",       ["Spec","UX","Metric","Loom","Backlog","Discovery","Release"]),
  d("eng",      "Engineering",      "68", "Builds Running",     ["Architecture","Backend","Frontend","Routing","Infra","QA","Release","Perf","Security"]),
  d("design",   "Design",           "15", "Assets in Flight",   ["System","Brand","Motion","Prototype","Review"]),
  d("data",     "Data & Analytics", "31", "Pipelines Live",     ["Pipeline","Warehouse","Model","Report","Quality","Insight"]),
  d("marketing","Marketing",        "42", "Active Campaigns",   ["Campaign","Paid","SEO","Lifecycle","Creative","Copy","Analytics"]),
  d("brand",    "Brand Studio",     "03", "Assets in Flight",   ["Identity","Voice","Social","Asset","Guideline"]),
  d("sales",    "Sales",            "56", "Pipeline Deals",     ["Outbound","Inbound","Demo","Close","Renewal","Forecast"]),
  d("growth",   "Growth",           "22", "Experiments Live",   ["Experiment","Funnel","Retention","Referral","Pricing"]),
  d("cx",       "Customer Ops",     "94", "Open Tickets",       ["Support","Success","Escalation","Docs","Feedback"]),
  d("content",  "Content",          "37", "Drafts in Queue",    ["Editorial","Longform","Social","Video"]),
  d("research", "Research",         "09", "Studies Running",    ["Literature","Interview","Synthesis","Competitive","Trend"]),
  d("infra",    "Infra / DevOps",   "11", "Services Monitored", ["Deploy","Observability","Cost","Runtime","Network","Backup"]),
  d("qa",       "QA & Verification","06", "Suites Passing",     ["Verification","Triple Pass","Reflection","Contract"]),
  d("memory",   "Knowledge",        "88", "Nodes Indexed",      ["Graph","Retrieval","Curation","Namespace","Resolution"]),
];
...
function buildLayout(): { placed: Placed[]; stars: { x: number; y: number; r: number; o: number }[] } {
...
export default function YvonGraph() {
  const { placed, stars } = useMemo(buildLayout, []);
    ...
    <div style={S.root}>
      ...
      <div style={S.hud}>
        ...
        <div style={{ display: "flex", gap: 5, pointerEvents: "auto" }}>
          {["YVON", "Novizio", "Hourbour", "AgentX"].map((s, i) => (
            <button key={s} style={{ ...S.tab, ...(i === 0 ? S.tabOn : {}) }}>{s}</button>
          ))}
        </div>
      </div>
