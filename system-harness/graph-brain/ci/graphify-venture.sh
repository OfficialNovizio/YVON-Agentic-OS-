...
mkdir -p graph
cp graphify-out/graph.json graph/graph.json
cat > README.md <<EOF
# yvon-graph
...
- \`graph/graph.json\` — structural code graph ($NODE_COUNT nodes, $EDGE_COUNT edges, $COMMUNITY_COUNT communities)
- \`knowledge/\` — semantic knowledge extracted from this repo (MemPalace)

Last built: $(date -u +%Y-%m-%dT%H:%M:%SZ)
EOF
git add graph/graph.json README.md
