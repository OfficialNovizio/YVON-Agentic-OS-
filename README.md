# yvon-graph

Auto-generated by YVON's onboarding pipeline. Do not edit by hand — this
branch is rebuilt on every graphify run and never merges into your default
branch.

- `graph/` — full graphify output (11604 nodes, 21474 edges, 1077 communities) — see `graph/graph.html` for the interactive visualization, `graph/GRAPH_REPORT.md` for the human-readable summary
- `knowledge/` — semantic knowledge extracted from this repo (MemPalace)

Last built: 2026-09-04T01:21:32Z
