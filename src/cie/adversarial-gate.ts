...
import { resolveToolBinding, type ToolBindingResult } from './tool-binding'
...
export function evaluateAdversarialGate(
  ragResult: RagRetrieveResult,
  department: string,
): AdversarialGateResult {
  const findings = (ragResult.selected_chunks ?? []).filter((c) => c.adversary)
  const toolBinding = resolveToolBinding(department, 'ADVERSARIAL_TESTING')
  ...
  return {
    ...
    reason:
      'no adversary-flagged findings — per the inversion, silence is NOT automatically a pass. ' +
