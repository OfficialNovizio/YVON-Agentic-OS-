...
export type ToolLocation =
  | 'repo-in-process'      // node_modules, agent skills — imported/loaded directly
  | 'vps-venv'              // VPS Python venv — subprocess/API bridge (rag/core/bridge.py pattern)
  | 'on-demand-service'     // Docker service via cli/tool.sh — 12GB FIFO queue applies
  | 'mcp-relay'             // MCP server, spawned on demand, registered by relay
  | 'dropped'               // explicitly removed from the registry — do not use
...
const SECTION_LOCATION: { match: RegExp; location: ToolLocation }[] = [
...
function cleanToolName(cell: string): string {
  return cell
    .replace(/\*\*/g, '')
    .replace(/`/g, '')
...
function extractAlias(cell: string): string | null {
...
function getRegistry(): RegistryEntry[] {
  ...
  const path = join(config.teamsPath, 'Shared OS', 'tools', 'shared-tool-registry.md')
  ...
  let currentSection = ''
  let currentLocation: ToolLocation | null = null
  ...
  for (const line of lines) {
    ...
    if (!insideRegistrySection || !currentLocation) continue
    ...
    if (['tool', 'server'].includes(cells[0].toLowerCase())) continue
    ...
    if (alias) entries.push({ name: alias, location: currentLocation, section: currentSection, raw: line.trim() })
  }
...
export function invalidateToolRegistryCache(): void {
  cachedRegistry = null
}
 ...
 * guess a default).
 */
export function resolveToolLocation(toolName: string): RegistryEntry | null {
...
const BASELINE_TOOLS: Record<string, string[]> = {
...
const ARCHETYPE_TOOLS: Partial<Record<Archetype, string[]>> = {
...
export const SCRAPING_ESCALATION_CHAIN = ['crawl4ai', 'scrapegraphai', 'agent-reach', 'browser-use']

export interface ToolBindingResult {
  department: string
  archetype?: Archetype
  baseline: { tool: string; location: ToolLocation | 'unregistered' }[]
  taskSpecific: { tool: string; location: ToolLocation | 'unregistered' }[]
}

function withLocation(tools: string[]): { tool: string; location: ToolLocation | 'unregistered' }[] {
...
export function resolveToolBinding(department: string, archetype?: Archetype): ToolBindingResult {
...
// ---------------------------------------------------------------------------
// 1.5 — on-demand service status, for the 12GB-VPS FIFO queueing policy
// (resolved 2026-08-09: simple wait, no eviction, no timeout). This only
// covers the `on-demand-service` location category; repo-in-process tools
// need no check, vps-venv/mcp-relay have no equivalent status command here.
// ---------------------------------------------------------------------------
 ...
 */
export function checkRunningServices(): ServiceStatusResult {
  let proc
  try {
    proc = spawnSync('bash', ['cli/tool.sh', 'status'], {
      cwd: getConfig().projectRoot,
      encoding: 'utf-8',
      timeout: 10_000,
    })
  } catch (err) {
    return { checked: false, running: [], error: (err as Error).message }
  }
 ...
 */
export function resolveOnDemandService(toolName: string): { registered: boolean; running: boolean | null; note: string } {
