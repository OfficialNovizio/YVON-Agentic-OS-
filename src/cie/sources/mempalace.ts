...
function isEnvNoise(line: string): boolean {
 ...
 */
export function searchMemPalace(
  query: string,
  opts: { wing?: string; room?: string; results?: number; timeoutMs?: number } = {},
  ...
  let proc
  try {
    proc = spawnSync('mempalace', args, {
      encoding: 'utf-8',
      timeout: opts.timeoutMs ?? 15_000,
    })
  } catch (err) {
    return { available: false, hits: [], error: `spawn failed: ${(err as Error).message}` }
  }
  ...
  if (proc.signal === 'SIGTERM') {
    return { available: false, hits: [], error: `timed out after ${opts.timeoutMs ?? 15_000}ms` }
  }
 ...
 */
export function mineIntoMemPalace(
  dir: string,
  opts: { wing?: string; timeoutMs?: number } = {},
): MemPalaceMineResult {
  const args = ['mine', dir]
  if (opts.wing) args.push('--wing', opts.wing)
  ...
  try {
    ...
    return { available: false, filed: false, error: `spawn failed: ${(err as Error).message}` }
  }
  ...
  if (proc.signal === 'SIGTERM') {
    return { available: false, filed: false, error: `timed out after ${opts.timeoutMs ?? 30_000}ms` }
  }
