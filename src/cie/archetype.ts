...
export const ARCHETYPE_TABLE: Record<Archetype, Omit<ArchetypeInfo, 'archetype'>> = {
  SHALLOW_LOOKUP: {
...
export const DEPARTMENT_ARCHETYPES: Record<string, Archetype[]> = {
...
const ARCHETYPE_PATTERNS: Partial<Record<Archetype, RegExp>> = {
 ...
 */
export function classifyArchetype(message: string, department: string): ClassifiedArchetype {
  for (const [archetype, pattern] of Object.entries(ARCHETYPE_PATTERNS) as [Archetype, RegExp][]) {
    if (pattern.test(message)) {
      return { archetype, ...ARCHETYPE_TABLE[archetype], source: 'keyword' }
    }
  }

  const normalizedDept = department === 'Cyber Security' ? 'Cybersecurity' : department
  const deptDefaults = DEPARTMENT_ARCHETYPES[normalizedDept]
  const archetype = deptDefaults?.[0] ?? 'PRECISION_CRITICAL'
  return { archetype, ...ARCHETYPE_TABLE[archetype], source: 'department-default' }
}
